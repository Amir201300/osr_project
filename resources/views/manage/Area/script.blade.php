...
<script>
          ...
          url = save_method == 'add' ? "{{route('Area.store')}}" : "{{route('Area.update')}}" ;
          $.ajax({
          url : url,
          type : "post",
          data : formData,
...
<script>
    ...
    function yesDelete()
    {
      ...
      if(checkNum == 1){
      deleteProccess("/manage/Area/delete/" +id_num);
      }else{
        
      deleteProccess("/manage/Area/delete/"+checkArray +'?type=2');
