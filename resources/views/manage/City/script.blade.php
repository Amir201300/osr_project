...
<script>
          ...
          url = save_method == 'add' ? "{{route('City.store')}}" : "{{route('City.update')}}" ;
          $.ajax({
          url : url,
          type : "post",
          data : formData,
...
<script>
    ...
    function yesDelete()
    {
      ...
      if(checkNum == 1){
      deleteProccess("/manage/City/delete/" +id_num);
      }else{
        
      deleteProccess("/manage/City/delete/"+checkArray +'?type=2');
