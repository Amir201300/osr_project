...
<script>
    function addFunction()
    {
        ...
        $('#title').text('{{trans("اضافة محافظة جديد")}}');
...
<script>
          ...
          url = save_method == 'add' ? "{{route('Advices.store')}}" : "{{route('Advices.update')}}" ;
          $.ajax({
          url : url,
          type : "post",
          data : formData,
...
<script>
    ...
    function yesDelete()
    {
      ...
      if(checkNum == 1){
      deleteProccess("/manage/Advices/delete/" +id_num);
      }else{
        
      deleteProccess("/manage/Advices/delete/"+checkArray +'?type=2');
