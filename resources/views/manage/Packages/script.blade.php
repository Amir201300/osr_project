...
<script>
    function addFunction()
    {
        ...
        $('#title').text('{{trans("اضافة باقه جديده")}}');
...
<script>
          ...
          url = save_method == 'add' ? "{{route('Packages.store')}}" : "{{route('Packages.update')}}" ;
          $.ajax({
          url : url,
          type : "post",
          data : formData,
...
<script>
    ...
    function yesDelete()
    {
      ...
      if(checkNum == 1){
      deleteProccess("/manage/Packages/delete/" +id_num);
      }else{
        
      deleteProccess("/manage/Packages/delete/"+checkArray +'?type=2');
