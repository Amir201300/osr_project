...
<script>
    function addFunction()
    {
        ...
        $('#title').text('{{trans("اضافة كورس جديد")}}');
...
<script>
          ...
          url = save_method == 'add' ? "{{route('Courses.store')}}" : "{{route('Courses.update')}}" ;
          $.ajax({
          url : url,
          type : "post",
          data : formData,
...
<script>
    ...
    function yesDelete()
    {
      ...
      if(checkNum == 1){
      deleteProccess("/manage/Courses/delete/" +id_num);
      }else{
        
      deleteProccess("/manage/Courses/delete/"+checkArray +'?type=2');
